...
import Boat.Cli qualified as Cli
import Boat.Dataflow qualified as Dataflow
import Boat.Dataflow.Label qualified as Dataflow.Label
import Boat.Driver qualified as Driver
import Boat.Error (CompileFail)
import Boat.LL qualified as LL
import Boat.LL.AstToIr qualified as LL.AstToIr
import Boat.LL.Lexer qualified as LL.Lexer
import Boat.LL.Pass.Live qualified as LL.Pass.Live
import Boat.Main qualified as Main
import Boat.Reporter qualified as Reporter
import Boat.Utils.IO (hPutLnUtf8, listDirectory', readFileUtf8, runErrorIO, writeFileLnUtf8)
import Boat.Utils.Impossible (impossible)
import Boat.Utils.Misc (timSort)
...
import qualified Boat.Dataflow.Fuel as Dataflow.Fuel
...
llCompileSpec config = do
  makeSnapshotSpec
    ...
    ( \text -> Temporary.runTemporary $ do
        Temporary.withSystemTempFile "boat" $ \temp handle -> do
          liftIO $
            Main.runDriverMain_
              Cli.defArgs
